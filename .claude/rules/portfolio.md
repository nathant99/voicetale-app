...
### R-RESOURCE-FIX-REDISTRIBUTE — an in-place FIX to a hub-owned resource is SOURCE-ONLY until re-distributed to EVERY consuming surface; verify the fix on the CONSUMERS, not just the hub source (2026-07-30)

**When you FIX/EDIT a hub-owned RESOURCE in place at the SOURCE (a question kit in hub `Resources/Questions/`, an illustration, a humor bank, a config), the fix is NOT shipped until it is RE-DISTRIBUTED to EVERY consuming surface in the SAME wave — the ~app repos (`Resources/Questions/<app>/` via `copy_<kind>_to_repos.sh`) AND the web (`public/play/<app>/…` via the porter, where mounted). A hub-source fix whose distribution step is skipped is SOURCE-ONLY: the hub `Resources/` reads clean, but the iOS fleet (and any un-re-ported web clone) keeps shipping the STALE, unfixed content indefinitely, with ZERO signal. Verify a resource fix's COMPLETENESS by grepping the CONSUMERS (app repos + web) for the fixed pattern — "0 hits in the hub source" is necessary-but-NOT-sufficient; the fix isn't done until the consumers are clean too.** Codified per founder-direct 2026-07-30 (*"verify, track and codify everything"*). This is the DISTRIBUTION-completion sibling of § R-HUB-OWNS-APP-RESOURCES (hub owns the resource's full lifecycle — including shipping the fix to the consumers) and a member of the "fixed-at-source ≠ shipped" family alongside § Asset Consumer Audit ("registered ≠ wired"), `spark-anvil-website.md` § R-WEB-CLONE-NARRATIVE-INTEGRATION ("authored ≠ integrated"), and § R-ASSET-GEN-COMPLETENESS ("generated ≠ shipped-and-visible").

**The incident (why this rule exists) — VERIFIED 2026-07-30.** The V610 filler-strip (2026-07-25, `strip_kit_filler_distractors.py`, "31,638 filler-tail distractors stripped portfolio-wide") + the V614 length-cue trim fixed the hub `Resources/Questions/` SOURCE + the web `public/play/<app>/kits/` (site #1378) — but **NEVER re-synced the iOS APP REPOS**. A portfolio scan found **104 of 195 active app repos still carrying 35,039 stale filler-tail occurrences** (wildlens 738, effectsforge 728, fossilforge 650, aiforge 634, …) — the iOS fleet had been shipping the filler-padded distractors for ~5 days, invisibly, while the hub + web read clean. Surfaced only when the V643/kit-quality session READ a live app-repo kit and found boilerplate suffixes (*"and this principle applies across similar situations"*, *"as this directly follows from the underlying principles"*) the hub source no longer had. **The fix = the skipped distribution step: `copy_kits_to_repos.sh --apply`** (re-distributes the cleaned hub kits to all stale app repos; resilient + registry-sourced + per-repo pull-then-push).

**The load-bearing disciplines:**
1. **Distribute in the SAME wave as the source fix.** Any pass that edits hub `Resources/` in place (a strip, a trim, a correction, an enrichment) MUST run `copy_<kind>_to_repos.sh --apply` (+ the web re-port for mounted surfaces) before it is called done — exactly as an asset-GENERATION wave files its handoff + distributes (§ Asset generation ownership). A "fixed the hub source" claim is a HALF-ship.
2. **Verify on the consumers with a POSITIVE re-scan — the strip's static FILLER pattern-list is NON-EXHAUSTIVE, so "I ran the strip" ≠ clean (2026-07-30, V677 bridgeforge).** The completeness check greps the app repos + web for the fixed pattern (expect 0), NOT just the hub source, AND NOT "did the strip run?" — because `strip_kit_filler_distractors.py`'s `FILLER` list is a fixed set of ~8 phrases, so a variant tail NOT in the list survives the strip silently. bridgeforge's 2 residual filler tails (*"…and this principle applies across similar situations"*) survived the V610 fleet strip in the HUB SOURCE itself (the app copy was byte-identical, not stale) → a post-strip re-scan on the source + consumers is what caught them (`--auto` template derivation catches variants the static list misses). Re-run the correctness gate (`audit_kit_questions.py`, E9) on the distributed copies after any strip/edit — a distribution can de-mask a latent defect.
3. **⚠ The DUPLICATE-KIT-SET gotcha + determine CANONICITY from the wired index, never a filename guess (2026-07-30, V677 fossilforge — CORRECTED).** A few app repos carry TWO overlapping kit-numbering schemes for the same slots (fossilforge: `kit_01_fossil_types_*` AND `kit_01_what_are_fossils_*`, both opening "What is a fossil?" — the `audit_kit_questions.py` **E8 "same prompt … answer reworded"** warning is the tell). **CANONICITY is determined by GROUND TRUTH — the app's WIRED index, i.e. the `/play` web clone's `kits-index.ts` (ported from iOS per R-WEB-CLONE-PARITY), NOT by which filename "looks" canonical.** For fossilforge the wired arc is the **`what_are_fossils…` scheme** (16 kits, one per slot) → the `fossil_types…` scheme is the superseded ORPHAN set (the earlier version of THIS rule had these reversed — R-CANONICAL-DOC-GROUND-TRUTH). `copy_kits_to_repos.sh` copies hub→app but does NOT delete app-only files, so it leaves the orphan set in place. Resolve by REMOVING the orphan set from hub source + app repo (hub-owned resource op, R-HUB-OWNS-APP-RESOURCES; `git rm` + a `HANDOFF_FROM_HUB_*_KIT_DEDUP.md` recording the canonical arc so the app session drops any lingering Swift reference) — NOT by re-running the sync. Reference: V677 removed fossilforge's 8 orphan kits (24→16, E9 0 errors); `fossilforge-app/Docs/HANDOFF_FROM_HUB_FOSSILFORGE_KIT_DEDUP.md`.
4. **When it applies:** any in-place fix to a hub-distributed resource; any "I fixed X in the hub" claim (ask: is it re-distributed + verified on the consumers?); a periodic drift check (grep the app repos for a known-fixed pattern). **Cross-refs:** § R-HUB-OWNS-APP-RESOURCES · § Asset generation ownership + handoff requirement · § Asset Consumer Audit · § R-ASSET-GEN-COMPLETENESS · § R-KIT-DISTRACTOR-LENGTH-CUE (the fix whose distribution this completes) · `scripts/copy_kits_to_repos.sh` + `scripts/audit_kit_questions.py` · `Docs/AUDIT_KIT_FILLER_APP_REPO_SYNC_LAG_2026-07-30.md` (the verified incident) · work-queue V674.
...
### R-KIT-DISTRACTOR-LENGTH-CUE — the MCQ length cue is fixed in TWO phases (safe-trim DONE + never-pad distractor ENRICHMENT); NEVER pad, always re-audit E9, worst-first, honest-yield (2026-07-30)

**When a question-kit MCQ's CORRECT answer is conspicuously the LONGEST option (`LENGTH_CUE`, Haladyna test-wiseness cue C7 — a test-wise learner can pick the fullest option and be right without knowing), the fix is a TWO-PHASE discipline, and BOTH phases are HARD-guarded against making the bank worse. A kit whose keys systematically out-length their distractors is a VALIDITY/measurement quality defect (not a correctness bug) — fixable, but ONLY by the disciplined method below; a blind length-balancing pass that PADS distractors is itself a defect (it recreates the filler artifact a prior pass stripped).** Codified per founder-direct 2026-07-30 (*"fix it and codify … add to work queue and prioritize"*), promoting the `AUDIT_KIT_RUBRIC_CUE_TRIAGE_2026-07-25.md` Finding-2 methodology + `RESEARCH_MCQ_ITEM_WRITING_RUBRIC_2026-07-25.md` into a standing rule. This is the kit-quality sibling of § R-HUMOR-SEED-COVERAGE (humor-bank correctness) and the enrichment half of the V610 kit-quality program.

**Phase 1 — SAFE TRIM (automatable, DONE fleet-wide V614).** Where a key is long ONLY because of a *separable trailing-elaboration* clause (`", such as …"` / `" — …"` / a parenthetical), MOVE that clause into the item's `explanation` field — balances option length with ZERO fabrication AND enriches feedback where elaborated feedback pedagogically belongs. Tool: **`scripts/trim_kit_length_cue.py`** (conservative + in-session-reviewed; guards against the 3 bad-trim classes: parallel-structure `compare/classify/difference` answers, inflection-blind stem match, paired mid-sentence em-dash). Shipped hub #2512 + site #1378 (~2,160 hub + 2,697 web items; LENGTH_CUE 43,351→41,191). This slice is ~5% of the cues — the *trailing-elaboration* sub-case only.

**Phase 2 — DISTRACTOR ENRICHMENT (per-item in-session-Opus authoring; the ~41k residual).** The rest are **inherently-fuller correct answers** (the key legitimately states the complete concept; distractors are terse). The ONLY honest fix is authoring **substantively-comparable, parallel-form, plausible-MISCONCEPTION distractors** — richer wrong options that a learner who holds the misconception would genuinely pick. The HARD guardrails (a violation makes the bank worse):
1. **🛑 NEVER PAD to balance length.** Padding a distractor with filler to match the key recreates the exact filler artifact the V610 strip + V614 trim removed. Enrich with *substance* (a real misconception elaborated), or LEAVE the item.
2. **LEAVE the legitimately-fuller-key items.** The flagger flags ~90–96% of items, but most are legitimate (a full-concept key + reasonable terse distractors is FINE — length alone is not a defect). Fixing only manufactures churn. The true fixable subset (terse/implausible distractors that ARE a giveaway) is a SMALL fraction — the honest per-item read decides, and "no safe non-padding fix" → leave it.
3. **ALWAYS re-audit E9 after ANY edit.** A trim or a re-authored distractor can DE-MASK a latent also-correct/duplicate defect (a shortened key equal to a distractor; an enriched distractor now also-correct). Re-run **`audit_kit_questions.py`** (0 hard errors) as the gate after every touch — same lesson as the filler-strip + trim.
4. **WORST-FIRST + OPPORTUNISTIC, not a blind 41k campaign.** Triage with **`flag_kit_rubric_cues.py --triage`** (fossilforge 518 → harvestforge → mapforge …); enrich the genuinely-terse subset per app, re-audit, distribute to BOTH surfaces (hub `Resources/Questions/<app>/` + web `public/play/<app>/kits/` re-port — hub owns both, § R-HUB-OWNS-APP-RESOURCES), flip the `Docs/REGISTRY_KIT_QUALITY_COVERAGE.txt` ledger. It is a multi-session drain.
5. **FORWARD-FIX is already in place** — the generation-time MCQ rubric (`RESEARCH_MCQ_ITEM_WRITING_RUBRIC_2026-07-25.md`) directs new kits to author comparable-length distractors, so NEW kits do not accrue the cue.
6. **HONEST YIELD — a validity quality-LIFT, NEVER an efficacy claim.** The items are already CORRECT; the cue only lets a *test-wise* learner guess on a low-stakes formative on-device app. Improve it as a measurement-quality bar; never claim a learning-outcome gain.

**When it applies:** any kit-quality LENGTH_CUE remediation; authoring/reviewing any new kit (apply the forward-fix); the tracked enrichment drain (work-queue V674, prioritized). **Cross-refs:** `Docs/AUDIT_KIT_RUBRIC_CUE_TRIAGE_2026-07-25.md` + `Docs/RESEARCH_MCQ_ITEM_WRITING_RUBRIC_2026-07-25.md` + `Docs/CONTEXT_HANDOFF_2026-07-25_V614_LENGTH_CUE_TRIM.md` + `Docs/AUDIT_QUESTION_KIT_QUALITY_mapforge_2026-07-30.md` · `scripts/{flag_kit_rubric_cues,trim_kit_length_cue,audit_kit_questions}.py` · `Docs/REGISTRY_KIT_QUALITY_COVERAGE.txt` · § R-HUMOR-SEED-COVERAGE + § R-ASSET-GEN-COMPLETENESS (sibling content-quality guards) · § R-HUB-OWNS-APP-RESOURCES (hub edits kit JSONs directly) · `spark-anvil-website.md` § R-WEB-CLONE-KITS-OPUS-AUTHOR (the web-kit authoring sibling) · `workflow.md` § R-INSESSION-OPUS-FREE (the free-Opus posture that funds the enrichment).
...
## "Is app X ready to start implementation?" is a docs-readiness verification against the ACTUAL Docs/ tree — NEVER the app's own DOCUMENT_CATALOG, and the absence of Package.swift/.xcodeproj/.swift is the EXPECTED pre-implementation state (R-APP-IMPL-READINESS-CHECK; 2026-07-30)

**When the founder asks "does app X have everything / is it ready to start implementation?", the answer is a docs-readiness verification derived from GROUND TRUTH — a fresh `git pull` + a re-inventory of the app repo's ACTUAL `Docs/` tree, `.claude/rules/`, and `Resources/` — NOT from the app's own `DOCUMENT_CATALOG.md` (a stale-prone SECONDARY index — R-CANONICAL-DOC-GROUND-TRUTH). The verdict is GREEN when the ENTRY-POINT set + content assets + scaffold are present; and the ABSENCE of `Package.swift` / `.xcodeproj`/`.xcworkspace`/`.pbxproj` / `.swift` source is the CORRECT, EXPECTED pre-implementation state (writing those is the app's OWN CC session's first task per § R-APP-SPAWN-PLAYBOOK / CLAUDE.md § CRITICAL — hub never writes Swift/Xcode source), so it is NEVER reported as a "missing doc" or a gap.** Codified 2026-07-30 (founder "codify everything") after a TaleForge readiness check: the repo was fully docs-ready, but its `DOCUMENT_CATALOG.md` was 2.5 months stale (dated 2026-05-12, "~50 docs / 18 rules") — omitting every `HANDOFF_FROM_*` doc, the `dn-s/` chapters, `APP_SPECIFIC_NOTES`, `PLAYBOOK_MONOREPO_REORGANIZATION`, and 13 of 31 `.claude/rules` — so trusting the catalog would have both under-counted the corpus AND missed that the app is ready.

**The readiness checklist (verify each against the on-disk tree, not the catalog):**
1. **Entry-point docs present:** `Docs/IMPLEMENTATION_HANDOFF.md` (Phase-1 scope + SPM-target spec — the START-HERE) · `Docs/TECHNICAL_DESIGN.md` (architecture + data models) · `Docs/PLAN_FULL_APP_IMPLEMENTATION.md` · a ForgeKit-bootstrap handoff (`HANDOFF_FROM_{HUB,LABSMITH}_FORGEKIT_BOOTSTRAP.md`).
2. **Scaffold present:** `CLAUDE.md` + the `.claude/rules/` set (spot-count, don't trust the catalog's count) + `ExportOptions.plist` + `PrivacyInfo.xcprivacy`.
3. **Content assets distributed** (the hub-owned half of the spawn): question kits under `Resources/Questions/`, cast portraits, DN-S chapters under `Docs/dn-s/chapters/`, illustrations/audio (the § R-SPAWN-COMPLETENESS-DIMENSIONS content dims).
4. **Expected-absent (NOT a gap):** `Package.swift` · any `.xcodeproj`/`.xcworkspace` · `.swift` source — these are the app-session's Phase-1 deliverable; `Libraries/Sources/` existing WITHOUT a `Package.swift` is the normal scaffolded-but-not-yet-built state.
5. **If the catalog is stale, REFRESH it in the same pass** (a hub-owned `Docs/*.md` edit — the R-CANONICAL-DOC-GROUND-TRUTH "reconcile the drifted source, don't just avoid copying it" discipline): rebuild `DOCUMENT_CATALOG.md` from the re-inventory.

**When it applies:** any "is app X ready / does it have all the docs?" question; any spawn-readiness review. Run the § R-SPAWN-COMPLETENESS-DIMENSIONS guards for the machine-checkable dims (cast/clone/kits). **Cross-refs:** § R-APP-SPAWN-PLAYBOOK (what a spawn delivers) · § R-SPAWN-COMPLETENESS-DIMENSIONS (the 11 per-app dims + their guards) · `workflow.md` § R-CANONICAL-DOC-GROUND-TRUTH (verify against on-disk reality; a catalog is a stale-prone secondary doc) · CLAUDE.md § CRITICAL (hub never writes Swift/Xcode source — the reason Package.swift-absence is expected).
