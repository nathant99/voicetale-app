...
2. **INTUITIVE** — **pedagogical chapter order** (§ R-PDF-BOOK-PEDAGOGICAL-ORDER, `dnCast.members[]` arc), a clean heading hierarchy (one title level per chapter; front/back matter consistently indented), and **each chapter on a fresh page** (a clear "sense of arrival" + a natural pause). No raw-markdown artifacts leaking into the render (e.g. an empty `## ` heading rendering as literal `##`).
...
5. **REGISTER/TRUST HYGIENE** — reader-facing pages (Copyright/About/License) carry NO internal engineering register — no project ADR numbers, no pre-rename repo names (`labsmith`), no ticket IDs (the same stoplist as § R-CHAPTER-REGISTER / `spark-anvil-website.md` § R-SITE-CHROME).
