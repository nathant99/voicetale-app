...
- **`.process("Resources")` FLATTENS subdirectories → subdirectory lookups return nil (R-SPM-PROCESS-FLATTEN-FALLBACK)** — a target that declares `.process("Resources")` **collapses the entire `Resources/` subtree to the bundle ROOT**. A file committed at `Resources/Cast/beats/chapter_dot_beat_00.webp` ships in the bundle as `chapter_dot_beat_00.webp` (no `Cast/beats/` path). So `Bundle.module.url(forResource: name, withExtension: ext, subdirectory: "Cast/beats")` returns **nil** → the asset silently renders as a gray placeholder / fails to load, with **no build error** (SPM doesn't validate `subdirectory:` at build time). **Every `Bundle.module.url(..., subdirectory:)` lookup in a `.process(...)` target MUST have a flat fallback:**
  ```swift
  let url = Bundle.module.url(forResource: name, withExtension: ext, subdirectory: "Cast/beats")
         ?? Bundle.module.url(forResource: name, withExtension: ext)   // flat fallback — this is the one that hits under .process
  ```
  Because flatten drops all files to one namespace, resource filenames MUST be globally unique across every subdirectory the target processes (prefix by kind + slug, e.g. `chapter_<slug>_beat_NN.webp` vs the flat portrait `<slug>.webp`). **Alternative** when you genuinely need the subtree structure (e.g. ForgeAudio's `resolveBundleURL` walks `AudioDramas/<app>/`): use a **`.copy("DirName")` rule** (a sibling to `.process("Resources")`) — `.copy` preserves the directory structure verbatim, so `subdirectory:` lookups resolve. **Grep discipline**: before shipping any new bundled-resource lookup, `grep -rn 'subdirectory:' Sources/` and confirm each has a flat fallback OR lives under a `.copy` directory. Codified 2026-07-06 after FractionForge hit this **3×** (Q12 audio → `.copy("AudioDramas")` fix; 2026-07-02 cast portraits → flat fallback; 2026-07-06 beat illustrations → flat fallback). Reference: `fractionforge-app/Docs/APP_SPECIFIC_NOTES.md` § "Cast portraits render as gray placeholders". Sibling gotcha: the empty-resource-dir codesign bullet above (same `.process` mechanism).
