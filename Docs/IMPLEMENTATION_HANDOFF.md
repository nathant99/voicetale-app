...
>
> **Re-affirmed 2026-06-22** (third consecutive multi-commit auto-cycle round): the rule applies to EVERY new feature branch — agent never authors `.xcworkspace` / `.xcscheme` / `.xctestplan` / `Info.plist` / `.entitlements` bytes from disk. If a Phase 1.x feature needs one of those, file a `Docs/HANDOFF_TO_USER_<TOPIC>.md` before merging the feature branch.
