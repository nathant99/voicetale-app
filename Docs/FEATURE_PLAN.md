...
- [x] **Share-worthy moments — mood-tag retrospectives** — `Models/MoodRetrospective` pure-function value enum (3-tier: 3-9 / 10-24 / 25+) ships 12 per-mood Bramble-register one-liners (4 moods × 3 tiers); `AnthologyView.moodRetrospectiveCard` surfaces above the filtered tale list when (a) a mood filter is applied AND (b) ≥3 tales of that mood exist. 8 MoodRetrospectiveTests lock tier mapping + copy presence matrix + below-floor nil contract + anti-shame token guard + count interpolation. PR #88 (2026-06-23). Published-tale certificates + anthology covers remain deferred.
